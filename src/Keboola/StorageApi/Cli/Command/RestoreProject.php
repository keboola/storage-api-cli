    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($buckets as $bucketInfo) {
            ...
            if ($input->getOption('ignore-storage-backend')) {
                ...
                $client->createBucket($bucketName, $bucketInfo['stage'], $bucketInfo['description'], $bucketInfo['backend']);
            }
        ...
        foreach ($tables as $table) {
            ...
            if (count($slices["Contents"]) == 1 && substr($slices["Contents"][0]["Key"], -14) != '.part_0.csv.gz') {
                ...
                $client->createTableAsyncDirect($table["bucket"]["id"], [
                    "name" => $table["name"],
                    "dataFileId" => $fileId,
                ...
                $tableId = $client->createTable($table["bucket"]["id"], $table["name"], $headerFile, ["primaryKey" => join(",", $table["primaryKey"])]);

                // Upload data to table
                $client->writeTableAsyncDirect($tableId, array(
